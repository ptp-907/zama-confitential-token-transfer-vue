        ...
        <div class="sessions-list">
          ...
          <div v-else class="sessions-grid">
            ...
            >
              ...
              >
                <svg v-if="isRequestingTally || isDecrypting" class="icon animate-spin" fill="none" viewBox="0 0 24 24">
                  <circle class="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" stroke-width="4"/>
                  <path class="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4zm2 5.291A7.962 7.962 0 014 12H0c0 3.042 1.135 5.824 3 7.938l3-2.647z"/>
                </svg>
                <svg v-else class="icon" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                ...
                {{ session.revealRequested ? 'Decrypt & Submit Tally' : 'Request Tally Reveal' }}
              </button>
            </div>
          </div>
...
// Handle create session with topic caching
const handleCreateSession = async () => {
  if (!sessionTopic.value.trim()) return
  ...
  try {
    ...
    const newCachedSession = {
      ...
      duration: newSessionDuration.value,
      createdAt: new Date().toISOString()
    }
    
    // Ensure cachedSessions.value is an array before spreading
    if (!Array.isArray(cachedSessions.value)) {
...
const decryptTally = async (sessionId: number) => {
  ...
  try {
    ...
    // Decrypt both handles using decryptMultipleHandles
    const result = await decryptMultipleHandles(VOTING_CONTRACT_ADDRESS, signer, [yesVotesHandle, noVotesHandle])
...
const requestTallyReveal = async (sessionId: number) => {
  ...
  try {
    ...
    // Step 3: Decrypt both handles using decryptMultipleHandles
    isDecrypting.value = true
    props.onMessage('Decrypting encrypted votes (this may take a moment)...')
    ...
    if (error.message?.includes('User rejected')) {
      ...
      // If voting hasn't ended, just request the reveal (don't decrypt yet)
      props.onMessage('Voting session has not ended yet. Tally reveal requested.')
      await loadSessions()
      // Try to decrypt if reveal was already requested
